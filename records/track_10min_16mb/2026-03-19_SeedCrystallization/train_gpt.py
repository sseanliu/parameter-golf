...
class Hyperparameters:
    ...
    eval_stride = int(os.environ.get("EVAL_STRIDE", 64))  # sliding window eval stride
    max_wallclock_seconds = float(os.environ.get("MAX_WALLCLOCK_SECONDS", 600.0))
    qk_gain_init = float(os.environ.get("QK_GAIN_INIT", 1.5))

    # Model shape.
    vocab_size = int(os.environ.get("VOCAB_SIZE", 1024))
    num_layers = int(os.environ.get("NUM_LAYERS", 12))  # effective depth (recycled)
    num_unique_blocks = int(os.environ.get("NUM_UNIQUE_BLOCKS", 3))  # actual unique blocks stored
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,iter_attn,iter_mlp,iter_resid",
    ).split(",")
    if pattern
)
...
class Block(nn.Module):
    """Shared transformer block. Scale/mix params are per-iteration (owned by GPT)."""
    def __init__(
        self,
        dim: int,
        num_heads: int,
...
class GPT(nn.Module):
    def __init__(
        ...
        self.num_layers = num_layers  # effective depth (recycled)
        ...
        # Shared blocks: only num_unique_blocks are stored, cycled across num_layers
        ...
        # Per-iteration scalars: each effective layer gets its own scale/mix params
        # These are tiny (~3 * dim * num_layers params) but break symmetry between recycled applications
        self.iter_attn_scales = nn.ParameterList(
            [nn.Parameter(torch.ones(model_dim, dtype=torch.float32)) for _ in range(num_layers)]
        )
        self.iter_mlp_scales = nn.ParameterList(
            [nn.Parameter(torch.ones(model_dim, dtype=torch.float32)) for _ in range(num_layers)]
        )
        self.iter_resid_mixes = nn.ParameterList(
    ...
    def _init_weights(self) -> None:
        ...
        for i in range(self.num_layers):
            with torch.no_grad():
                phase = torch.sigmoid(torch.tensor(3.0 * (i / max(self.num_layers - 1, 1) - 0.5)))
...
def main() -> None:
    ...
    # Add per-iteration scalars and skip weights to scalar optimizer
    for p in base_model.iter_attn_scales:
        scalar_params.append(p)
    for p in base_model.iter_mlp_scales:
        scalar_params.append(p)
    for p in base_model.iter_resid_mixes:
        scalar_params.append(p)
